...
from taskturbine.worker import execute_task
...
def test_create_worker_default_channels() -> None:
    worker = demo_app.create_worker("worker-1")
...
def test_claimedtask_retry_in_defaults(channel: str) -> None:
    demo_app.set_channels([channel])

    demo_app.spawn_task("ok-task", {"oid": 123}, channel=channel)
    worker = demo_app.create_worker("worker-1", [channel])
    claimed = worker._inner.claim_tasks()
...
def test_execute_task_error_handler(config: Config, channel: str) -> None:
    ...
    claimed = worker.claim_tasks()
    for msg in claimed:
        execute_task(app, msg)


def test_worker_run_batch_mixed_failure(
    db_connection: Connection, channel: str
) -> None:
    demo_app.set_channels([channel])
    first = demo_app.spawn_task("oid-partial-failure", {"oid": 123}, channel=channel)
